...
한국 채용 사이트(점핏·원티드·랠릿·잡코리아·사람인)에서 **신입 개발자 공고**를 모아
...
취업 준비 중 매일 여러 채용 사이트를 돌며 "신입 가능한가?"를 일일이 확인하는 게 비효율적이었습니다.
...
| 사람인 | **공개 API가 없어** 검색 페이지 HTML 파싱 | 제목 기준 직무·경력 필터 후 `신입/확인`(상세 확인 권장) |
...
- **직무·경력 노이즈 필터**: 랠릿 API의 직무 필터가 불완전해 프론트·디자인·기획이 섞여 들어오는 문제가 있었습니다. 그래서 반환된 공고의 **제목·스킬을 트랙 스택/키워드로 재검증**하고, 비개발 직무(영업·PM·컨설 등)·모바일앱·시니어/경력 공고는 제목 기준으로 걸러 노이즈를 크게 줄였습니다.
...
- 외부 API의 필터 파라미터를 **100% 신뢰하지 않고**(랠릿은 직무 필터가 새는 경우가 있었음) 클라이언트에서 제목·스택으로 재검증해 노이즈를 줄이는 방어적 설계
